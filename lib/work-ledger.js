...
export function workLedgerFromActivities(activities) {
  ...
  for (const activity of activities ?? []) {
    ...
    const summary = normalizeLedgerText(redactSensitiveText(activity?.summary))
    ...
    if (FILE_ACTIVITY_NAMES.has(name) && status !== 'failed') filesChanged.push(...(paths.length ? paths : summary ? [summary] : []))
    const commandRecord = COMMAND_ACTIVITY_NAMES.has(name) ? command || summary : ''
    if (commandRecord) {
      commands.push({
    ...
    const failureSummary = commandRecord || summary
    if (status === 'failed') failedAttempts.push(`${name || 'Tool'}${failureSummary ? ` · ${failureSummary}` : ''}`)
  }
  return normalizeWorkLedger({ version: WORK_LEDGER_VERSION, filesChanged, commands, failedAttempts })
}
