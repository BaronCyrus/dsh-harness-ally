window.__ModuleLoader__.load({
  ...
  const exports = module.exports
    ...
    const LABELS = { dsh: 'DeepSeek Harness', 'claude-code': 'Claude Code', codex: 'Codex' }
    const ICONS = {
      dsh: {
        color: '#5786FE',
    ...
    const PROVIDER_LABELS = { 'ally-claude-code': 'Claude Code', 'ally-codex': 'Codex' }
    ...
    function createController() {
      ...
      const current = (sessionId) => records.get(sessionId) ?? {
        harness: 'dsh',
        providers: { dsh: true, 'claude-code': false, codex: false },
        dispatches: {},
        loading: false,
        pending: false,
        error: undefined,
      ...
      return {
        ...
        async load(sessionId) {
          ...
          publish(sessionId, { loading: true, error: undefined })
          try {
            ...
            if (revisions.get(sessionId) === revision) {
              publish(sessionId, { loading: false, error: error.message })
            }
          }
      ...
      return {
        ...
        async load(sessionId) {
          ...
          try {
            ...
            if (revision === currentRevision) publish({ loading: false, error: error.message })
          }
        },
    ...
    function harnessIcon(harness, className = 'ally-engine-icon') {
      ...
      }, h('path', { d: icon.path, fill: 'currentColor', fillRule: 'evenodd', clipRule: 'evenodd' }))
    }
    ...
    function HarnessSelector({ sessionId, t, useSessions, useSession }) {
      const ally = useSessions((state) => state.current === sessionId
        && state.byId?.[sessionId]?.agentPreset === ALLY_PRESET)
      ...
      useEffect(() => { if (ally) controller.load(sessionId) }, [ally, sessionId, completedTurns])
      useEffect(() => { if (ally) cliController.load(sessionId) }, [ally, sessionId])
      ...
      return h('div', { className: 'ally-engine' },
        h('button', {
          type: 'button',
