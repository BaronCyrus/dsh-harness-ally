...
export const HARNESSES = Object.freeze(['dsh', 'claude-code', 'codex'])
const HARNESS_LABELS = Object.freeze({ 'claude-code': 'Claude Code', codex: 'Codex' })
...
export function createAllianceRuntime({ sessions, agents, gateway, state, isAgentLoopRequest }) {
  ...
  async function* route(options, next) {
    ...
    const prompt = harnessPrompt(options, harness)
    const dispatch = {
      ...boundary,
      runId,
      harness,
    ...
    try {
      ...
      const executorTitle = harness === 'claude-code' ? 'Claude Code' : 'Codex'
      ...
      if (run.stream) {
        for await (const event of run.stream) {
          ...
          if (event?.type === 'reasoning-delta' && typeof event.text === 'string' && event.text) {
            ...
            if (!name || seenActivities.has(activityId)) continue
