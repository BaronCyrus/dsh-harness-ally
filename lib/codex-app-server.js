...
function activityForItem(item, status) {
  ...
  return { type: 'activity', id: item.id, name, summary, ...(paths?.length ? { paths } : {}), status }
}
...
export async function startCodexAppServerRun(deps, request) {
  ...
  const onMessage = (message) => {
    ...
    if ((method === 'item/started' || method === 'item/updated' || method === 'item/completed')
      ...
      const snapshot = activity ? `${activity.name}\u0000${activity.summary}\u0000${activity.status}` : undefined
