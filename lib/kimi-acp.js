...
export async function startKimiAcpRun(deps, request) {
  ...
  const state = { text: '', stream, activityIds: new Set(), protocolErrors: 0 }
  ...
  const handleServerRequest = (message) => {
    if (message.method === 'session/request_permission') {
      ...
      const regularApproval = options.some((option) => option?.kind === 'allow_always')
      const outcome = regularApproval && allowOnce?.optionId
  ...
  const onUpdate = (params) => {
    ...
    if (update?.sessionUpdate === 'tool_call' || update?.sessionUpdate === 'tool_call_update') {
      const activity = activityForUpdate(update)
    ...
    if (update?.sessionUpdate === 'plan' && Array.isArray(update.entries)) {
      const current = update.entries.find((entry) => entry?.status === 'in_progress') ?? update.entries[0]
      if (typeof current?.content === 'string' && current.content) {
        state.stream.push({ type: 'activity', id: `plan:${current.content}`, name: 'Plan', summary: current.content, status: 'running' })
      }
    }
  }
  ...
  void (async () => {
    try {
      await sendRequest('initialize', {
        protocolVersion: 1,
        clientInfo: { name: 'dsh-ally', version: '0.8.0' },
      ...
      settle({
        output: state.text ? [{ type: 'text', text: state.text }] : [],
        stopReason: signal.aborted || disposing ? 'aborted' : 'error',
        ...(signal.aborted || disposing ? {} : { diagnostic: 'Kimi Code ACP 初始化失败' }),
      }, true)
    }
  })()
