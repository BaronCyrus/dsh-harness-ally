  ...
  const onUpdate = (params) => {
    ...
    if (update?.sessionUpdate === 'tool_call' || update?.sessionUpdate === 'tool_call_update') {
      ...
      if (isSkillActivity && activity.status === 'completed') {
        ...
        if (!isSkillActivity) state.freshSkillRecoverySafe = false
    ...
    try {
      ...
      if (recoverableSkillStop && state.skillRecoveryAttempts === 0 && !signal.aborted && !disposing) {
        ...
        if (recoveredFromTimeout && state.freshSkillRecoverySafe) {
          ...
          recoveryPrompt = freshSkillRecoveryPrompt(prompt, state.lastSkillName)
      ...
      if (response?.stopReason === 'end_turn' && !state.skillContinuationPending) {
        settle({ output, stopReason: 'completed' }, true)
      } else if ((response?.stopReason === 'cancelled' && !state.skillContinuationTimedOut) || signal.aborted || disposing) {
